...
class Game_Environment:
    ...
    def reset(self): 
        ...
        for i in range(self.enemy_count):
            ...
            while self.enemy_dict[f"enemy{i}"] == self.player or self.enemy_dict[f"enemy{i}"] in [self.food_dict[item] for item in self.food_dict]:
                self.enemy_dict[f"enemy{i}"] = character(self.size_x, self.size_x, 0, 0)   
    ...
    def get_image(self):
        if self.game_status == 0 or self.game_status == 1:
            image = np.zeros((self.size_x, self.size_y, 3), dtype=np.uint8) 
            for item in self.food_dict:
                image[self.food_dict[item].x][self.food_dict[item].y] = self.color_dict['Food']          
            image[self.player.x][self.player.y] = self.color_dict['Player']
            for item in self.enemy_dict:
                image[self.enemy_dict[item].x][self.enemy_dict[item].y] = self.color_dict['Enemy']          
            image[self.player.x][self.player.y] = self.color_dict['Player'] 
            img = Image.fromarray(image, 'RGB')
            
        if self.game_status == -1:
            image = np.ones((self.size_x, self.size_y, 3), dtype=np.uint8)
            image[:,:,:2] *= 0
            image[:,:,2] *= self.color_dict['Background Loss']
            image[self.food.x][self.food.y] = self.color_dict['Food']  
            img = Image.fromarray(image, 'RGB')
        return img
